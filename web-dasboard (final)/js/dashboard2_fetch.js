...
const createOrUpdateChart = (
  ...
  let dataPoints = laneData.map((entry) => {
    ...
    const timestamp = new Date(Datetime.replace(/ at /, " ")); // parse timestamp
    return { x: timestamp, y: Count };
  });
  ...
  onValue(dbRef, (snapshot) => {
    ...
    if (data) {
      ...
      const filteredLeftLaneData = filterDataByDate(
        data["Left Lane Vehicle Count"],
        selectedDate
      );
      const filteredRightLaneData = filterDataByDate(
        data["Right Lane Vehicle Count"],
        selectedDate
      );
      ...
      // initialize charts for the first vehicle class
